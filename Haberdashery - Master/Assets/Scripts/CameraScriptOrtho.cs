...
public class CameraScriptOrtho : MonoBehaviour
{
    ...
    private Camera mainCam;             // Used for referencing the camera
    private float zoomSpeed;            // Reference speed for the smooth damping of the orthographic size
    private Vector3 moveVel;            // Reference velocity for the smooth damping of the position
    private Vector3 desiredPos;         // The position the camera is moving towards
    ...
    private void Move()
    {
        ...
        transform.position = Vector3.SmoothDamp(transform.position, desiredPos, ref moveVel, dampTime);
    }
    ...
    private void Zoom()
    {
        ...
        mainCam.orthographicSize = Mathf.SmoothDamp(mainCam.orthographicSize, requiredSize, ref zoomSpeed, dampTime);
    }
    ...
    private float FindRequiredSize()
    {
        ...
        for (int i = 0; i < players.Length; i++)
        {
            ...
            size = Mathf.Max(size, Mathf.Abs(desiredPosToTarget.x) / mainCam.aspect);
        }
